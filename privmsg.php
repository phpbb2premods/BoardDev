...
$private_message = ( $board_config['allow_colortext'] && $privmsg['user_colortext'] != '' ) ? '<font color="' . $privmsg['user_colortext'] . '">' . $private_message . '</font>' : $private_message;
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	else 
	{
		...
		else if ( $mode == 'reply' || $mode == 'quote' )
		{
			...
			$sql = "SELECT pm.privmsgs_subject, pm.privmsgs_date, pmt.privmsgs_bbcode_uid, pmt.privmsgs_text, u.username, u.user_id, u.user_colortext
				FROM " . PRIVMSGS_TABLE . " pm, " . PRIVMSGS_TEXT_TABLE . " pmt, " . USERS_TABLE . " u
				WHERE pm.privmsgs_id = $privmsg_id
					AND pmt.privmsgs_text_id = pm.privmsgs_id
			...
			if ( $mode == 'quote' )
			{
				...
				if ( $board_config['allow_colortext'] && $privmsg['user_colortext'] != '')
				{
					if ( preg_match('/^[0-9A-Fa-f]{6}$/', $privmsg['user_colortext']) )
					{
						$privmsg['user_colortext'] = '#' . $privmsg['user_colortext'];
					}

					$privmsg_message = str_replace('[quote', '[/color][quote', $privmsg_message);
					$privmsg_message = str_replace('[/quote]', '[/quote][color=' . $privmsg['user_colortext'] . ']', $privmsg_message);

					$colortext_name = "#(\[quote=.(.*)\])(\[\/color\])#";
					$privmsg_message = preg_replace($colortext_name, "\\1", $privmsg_message);

					$colortext_start = '[/color]';
					$colortext_color = "#(\[color=.(.*)\].(.*)\[\/color\])#";
					$colortext_end = '[color=' . $privmsg['user_colortext'] . ']';
					$privmsg_message = preg_replace($colortext_color, $colortext_start . "\\1" . $colortext_end, $privmsg_message);

					$privmsg_message = str_replace('[color=' . $privmsg['user_colortext'] . '][color=', '[color=', $privmsg_message);

					$privmsg_message = '[quote="' . $to_username . '"][color=' . $privmsg['user_colortext'] . ']' . $privmsg_message . '[/color][/quote]';
...
$preview_message = ( $board_config['allow_colortext'] && $userdata['user_colortext'] != '' ) ? '<font color="' . $userdata['user_colortext'] . '">' . $preview_message . '</font>' : $preview_message;
