...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
	...
	if ( $search_results != '' )
	{
		...
		{
			$sql = "SELECT pt.post_text, pt.bbcode_uid, pt.post_subject, p.*, f.forum_id, f.forum_name, t.*, u.username, u.user_id, u.user_sig, u.user_sig_bbcode_uid, u.user_colortext
		...
		for($i = 0; $i < count($searchset); $i++)
		{
			...
			if ( $show_results == 'posts' )
			{
				if ( isset($return_chars) )
				{
					...
					$message = str_replace("\n", '<br />', $message);
					$message = ( $board_config['allow_colortext'] && $searchset[$i]['user_colortext'] != '' ) ? '<font color="' . $searchset[$i]['user_colortext'] . '">' . $message . '</font>' : $message;
