...
//-- fin mod : quick post es ---------------------------------------------------
		...
		$user_colortext = ( !empty($HTTP_POST_VARS['user_colortext']) ) ? trim(strip_tags( $HTTP_POST_VARS['user_colortext']) ) : '';
	...
	if( isset( $HTTP_POST_VARS['submit'] ) )
	{
		...
		if ($signature != '')
		{
			...
			if ( strlen($sig_length_check) > 999999999 )
			{ 
				$error = TRUE;
...
//-- modify
			...
			$sql = "UPDATE " . USERS_TABLE . "
				SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) . "', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_birthday = '" . $user_birthday . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", $aim) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_qp_settings = '" . $user_qp_settings . "', user_colortext = '$user_colortext', user_sig_bbcode_uid = '$signature_bbcode_uid', user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowavatar = $user_allowavatar, user_allowsignature = $user_allowsignature, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_allow_pm = $user_allowpm, user_notify_pm = $notifypm, user_popup_pm = $popuppm, user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_active = $user_status, user_warnings = $user_ycard, user_rank = $user_rank, user_boulet = '" . str_replace("\'", "''", $user_boulet) . "'" . $avatar_sql . "
				WHERE user_id = $user_id";
//-- fin mod : quick post es ---------------------------------------------------
//-- fin mod : birthday --------------------------------------------------------
...
//-- fin mod : quick post es ---------------------------------------------------
		$user_colortext = htmlspecialchars($this_userdata['user_colortext']);
...
//-- fin mod : birthday --------------------------------------------------------
			...
			$s_hidden_fields .= '<input type="hidden" name="user_colortext" value="' . $user_colortext . '" />';
...
//-- fin mod : quick post es ---------------------------------------------------
			...
			'BOULET_SELECT_BOX' => '<option value="0"' . (( $user_boulet_type == 0 ) ? ' selected="selected"' : '' ) . '>' . $lang['user_boulet_type_0'] . '</option><option value="1"' . (( $user_boulet_type == 1 ) ? ' selected="selected"' : '' ) . '>' . $lang['user_boulet_type_1'] . '</option><option value="2"' . (( $user_boulet_type == 2 ) ? ' selected="selected"' : '' ) . '>' . $lang['user_boulet_type_2'] . '</option><option value="3"' . (( $user_boulet_type == 3 ) ? ' selected="selected"' : '' ) . '>' . $lang['user_boulet_type_3'] . '</option>',
