...
function bbstyle(bbnumber) {
	...
	if (bbnumber == -1) { // Close all open tags & default button names
		...
		if (bbc_box_on && !is_ie)
		{
			if (document.forms[form_name].addbbcode10.value == "[*]") {
				document.forms[form_name].addbbcode10.src = "templates/bbc_box/styles/" + stylePath + "/ulist.gif"; // display ulist image - reddog
			tmp_help = ulist_help;
			ulist_help = e_help;
			e_help = tmp_help;
			}
			if (document.forms[form_name].addbbcode12.value == "[*]") {
				document.forms[form_name].addbbcode12.src = "templates/bbc_box/styles/" + stylePath + "/olist.gif"; // display olist image - reddog
			tmp_help = olist_help;
			olist_help = e_help;
			e_help = tmp_help;
			}
	...
	if ((bbnumber == 10) && (bbtags[10] != "[*]"))
	{
		...
		else
		{
			...
			if (bbc_box_on && !is_ie)
			{
				document.forms[form_name].addbbcode12.src = "templates/bbc_box/styles/" + stylePath + "/element.gif"; // display element image - reddog
			}
			tmp_help = olist_help;
			olist_help = e_help;
			e_help = tmp_help;
	...
	if ((bbnumber == 12) && (bbtags[12] != "[*]"))
	{
		...
		else
		{
			...
			if (bbc_box_on && !is_ie)
			{
				document.forms[form_name].addbbcode10.src = "templates/bbc_box/styles/" + stylePath + "/element.gif"; // display element image - reddog
			}
			tmp_help = ulist_help;
			ulist_help = e_help;
			e_help = tmp_help;
	...
	for(r = 0; r < 5; r++)
	{
		...
		for(g = 0; g < 5; g++)
		{
			...
			for(b = 0; b < 5; b++)
			{
				...
				document.write('<a href="javascript:bbfontstyle(\'[' + colorTag + '=#' + color + ']\', \'[/' + colorTag + ']\');" onmouseover="helpline(\'' + helpTag + '\');" onfocus="this.blur();"><img src="images/spacer.gif" width="' + width + '" height="' + height + '" border="0" alt="#' + color + '" title="#' + color + '" /></a>');
				document.writeln('</td>');
			}
			if (dir == 'v')
			{
