...
function avc_notify_email($mod_rows, $latest_version, $mod_link, $download_link, $author_notes)
{
    ...
    global $board_config, $emailer, $phpbb_root_path, $phpEx, $version_config;
    if ( !class_exists('emailer') )
    {
        include($phpbb_root_path . 'includes/emailer.'.$phpEx);
        $emailer = new emailer($board_config['smtp_delivery']);
...
function xml_version_check($mod_rows)
{
    ...
    $xml->set_filename($mod_rows['mod_domain_loc'] . '/' . $mod_rows['mod_file_loc'] . '/' . $mod_rows['mod_file_name']);

    //
    // Read all our data from the file and get it together in a nice,
    // organized array
    //
    $version_check['version_info_stable'] = $xml->read_node(array('avc_retrievable', 'stable', 'version'));
    $version_check['version_info_dev'] = $xml->read_node(array('avc_retrievable', 'development', 'version'));
    $version_check['mod_link_stable'] = $xml->read_node(array('avc_retrievable', 'stable', 'mod_link'));
    $version_check['mod_link_dev'] = $xml->read_node(array('avc_retrievable', 'development', 'mod_link'));
    $version_check['download_link_stable'] = $xml->read_node(array('avc_retrievable', 'stable', 'download_link'));
    $version_check['download_link_dev'] = $xml->read_node(array('avc_retrievable', 'development', 'download_link'));
    $version_check['author_notes_stable'] = $xml->read_node(array('avc_retrievable', 'stable', 'author_notes'));
    $version_check['author_notes_dev'] = $xml->read_node(array('avc_retrievable', 'dev', 'author_notes'));
